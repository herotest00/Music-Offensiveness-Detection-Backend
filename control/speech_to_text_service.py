...
class SpeechToTextService:
    ...
    def __convert_to_wav(self):
        wav_filepath = os.path.join(self.__audio_dir, f"{self.__filename.rsplit('.', 1)[0]}.wav")
        ...
        self.__set_filepath(wav_filepath)
    ...
    def __extract_vocals(self):
        ...
        shutil.move(os.path.join(self.__audio_dir, self.__demucs_model_name,
                                 f"{self.__filename.rsplit('.', 1)[0]}", 'vocals.wav'), self.__filepath)
